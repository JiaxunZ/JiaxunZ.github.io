---
permalink: /
title: "About Me"
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

Hi! I'm Jiaxun Zhang, a undergraduate student at the [University of Illinois at Urbana-Champaign](https://illinois.edu/). I am fortunate to be advised by [Prof. Ismini Lourentzou](https://isminoula.github.io), [Prof. Jiaxuan You](https://cs.stanford.edu/~jiaxuan/), and [Prof. Koustuv Saha](https://koustuv.com).
<!--
My CV is available [here](/files/Jiaxun%20Zhang.pdf).
-->

## Education

- B.S. in Statistics & Computer Science, University of Illinois at Urbana-Champaign, 2022-present

## Awards

- Fiddler Innovation Undergraduate Student Fellowship Award, UIUC, 2025
- Dean's List — Fall 2022, Spring 2023, Spring 2024, Spring 2025

<!--
## Research Interests

My research focuses on advancing the capabilities and safety of AI systems:

- **LLM/VLM Reasoning** — Enhancing reasoning abilities in large language and vision-language models
- **Multi-agent Systems** — Developing collaborative AI agent frameworks
- **Reinforcement Learning** — Applying RL techniques to improve AI decision-making
- **LLM Agent Safety and Robustness** — Ensuring reliable and safe AI agent behaviors
- **AI for Science** — Leveraging AI to accelerate scientific discovery
- **AI for Healthcare and Mental Health** — Applying AI to improve health outcomes

## News

- **[Nov 2025]** Two papers accepted to **EMNLP 2025** (one Main Conference with Oral recommendation, one System Demonstrations)!
- **[2025]** Received the **Fiddler Innovation Undergraduate Student Fellowship Award** from UIUC!
- **[2024]** Paper accepted to **ECCV 2024 Workshop**!

## Education

**University of Illinois at Urbana-Champaign**  
B.S. in Statistics & Computer Science | Expected May 2026  
GPA: 3.94/4.00

**Relevant Coursework:**
- *Computer Science:* Machine Learning, Natural Language Processing, Deep Learning for Computer Vision, Data Mining, Machine Learning Systems, Algorithms, Numeric Methods
- *Math & Statistics:* Statistical Modeling, Applied Bayesian Analysis, Statistics and Probability, Linear Algebra

## Awards

- **Fiddler Innovation Undergraduate Student Fellowship Award**, UIUC, 2025
- **Dean's List** — Fall 2022, Spring 2023, Spring 2024, Spring 2025

## Contact

Feel free to reach out to me at [jiaxunz2@illinois.edu](mailto:jiaxunz2@illinois.edu).
-->
